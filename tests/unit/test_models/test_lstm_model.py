...
@pytest.mark.skipif(not (HAS_LSTM and HAS_TORCH), reason="LSTM or PyTorch not available")
...
class TestSequenceCreation:
    ...
    def test_create_sequences(self):
        ...
        assert len(X) > 0
        assert X.shape[1:] == (seq_length, len(feature_cols))  # (n_seqs, seq_len, n_features)
        assert len(y) == len(X)
