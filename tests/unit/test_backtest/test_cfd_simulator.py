...
try:
    from thesis.backtest.cfd_simulator import (
        run_backtest,
        _simulate_trades,
        _calculate_metrics,
    )
...
@pytest.mark.skipif(
    not (HAS_BACKTEST and HAS_CONFIG), reason="Backtest or config not available"
)
class TestBacktestRunner:
    """Test cases for backtest runner."""

    @pytest.mark.slow
    def test_backtest_with_predictions(
        self, raw_ohlcv_data, mock_predictions_with_labels
    ):
        ...
        try:
            ...
            result = _simulate_trades(pl.DataFrame(df), config)

            # Verify results - function returns dict with trades list
...
class TestBacktestDataLeakage:
    ...
    @pytest.mark.critical
    def test_prediction_timing_integrity(
        self, raw_ohlcv_data, mock_predictions_with_labels
    ):
