...
# ---------------------------------------------------------------------------
# GRU temperature-scaling calibration: _compute_ece
# ---------------------------------------------------------------------------

from thesis.stage_4_training.gru.calibration import _compute_ece, _calibrate_model


@pytest.mark.unit
class TestComputeECE:
    def test_perfect_calibration_zero_ece(self) -> None:
        """All predictions correct with max confidence → ECE ≈ 0."""
        # Use enough samples to fill bins reliably
        probs = torch.tensor(
            [[0.99, 0.005, 0.005]] * 5
            + [[0.005, 0.99, 0.005]] * 5
            + [[0.005, 0.005, 0.99]] * 5
    ...
    def test_all_wrong_high_ece(self) -> None:
        """All predictions wrong with high confidence → ECE close to 1."""
        probs = torch.tensor([[0.05, 0.05, 0.9], [0.9, 0.05, 0.05], [0.05, 0.9, 0.05]])
        labels = torch.tensor([0, 1, 2])
        ece = _compute_ece(probs, labels, n_bins=10)
        assert ece > 0.8

    def test_uniform_probs(self) -> None:
        """Uniform probabilities → moderate ECE."""
        probs = torch.full((30, 3), 1.0 / 3)
    ...
    def test_custom_bins(self) -> None:
        ...
        ece = _compute_ece(probs, labels, n_bins=5)
        assert isinstance(ece, float)


# ---------------------------------------------------------------------------
# GRU temperature-scaling calibration: _calibrate_model
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestCalibrateModel:
    def _make_mock_model(self, n_classes: int = 3) -> tuple:
        """Create mock GRU model + classifier that produce deterministic logits."""
        model = MagicMock()
        classifier = MagicMock()

        # model(batch_x) → return batch_x as hidden states (identity)
        model.side_effect = lambda x: x
        # classifier(hidden) → return logits
        classifier.side_effect = lambda h: torch.randn(h.shape[0], n_classes)

        return model, classifier

    def test_returns_float_temperature(self) -> None:
        """Calibration returns a positive float."""
        model, classifier = self._make_mock_model()
        ...
        val_loader = DataLoader(ds, batch_size=10)

        T = _calibrate_model(model, classifier, val_loader, torch.device("cpu"))
        assert isinstance(T, float)
        assert T > 0

    def test_with_accelerator(self) -> None:
        """Calibration with accelerator passes through without device placement."""
        model, classifier = self._make_mock_model()
        from torch.utils.data import DataLoader, TensorDataset
        ...
        # Mock accelerator — when provided, batch_x.to(device) is skipped
        accelerator = MagicMock()
        T = _calibrate_model(
            model, classifier, val_loader, torch.device("cpu"), accelerator=accelerator
        )
        assert isinstance(T, float)
        assert T > 0

    def test_temperature_near_one_for_soft_logits(self) -> None:
        """For well-scaled soft logits, T should be a positive number."""
        model = MagicMock()
        classifier = MagicMock()

        def _mock_logits(h: torch.Tensor) -> torch.Tensor:
            n = h.shape[0]
            logits = torch.randn(n, 3) * 0.5
            return logits

        model.side_effect = lambda x: x
        classifier.side_effect = _mock_logits
        ...
        y = torch.arange(60) % 3
        ds = TensorDataset(X, y)
        val_loader = DataLoader(ds, batch_size=15)
