# CHƯƠNG 2. CƠ SỞ LÍ LUẬN VÀ CÔNG NGHỆ

## 2.1. Đặc tính dữ liệu tài chính
...
```mermaid
graph LR
    A[Giá tại thời điểm t] --> B{Giá chạm barrier nào trước?}
    B -->|Upper barrier<br/>TP| C[Long +1]
    B -->|Lower barrier<br/>SL| D[Short -1]
    B -->|Vertical barrier<br/>Hết horizon| E[Hold 0]
```
...
## 2.5. Walk-forward validation và kiểm soát leakage
...
graph LR
    subgraph Window 1
        direction LR
        T1[Train] --> P1[Purge] --> E1[Embargo] --> V1[Validation]
    end
    subgraph Window 2
        direction LR
        T2[Train] --> P2[Purge] --> E2[Embargo] --> V2[Validation]
    end
    Window 1 --> Window 2
```
...
Trong báo cáo, cần nhấn mạnh rằng kiểm soát leakage là một phần đóng góp chính. Nếu chỉ dùng một mô hình mạnh nhưng validation sai, kết quả có thể cao nhưng không có giá trị khoa học.

## 2.6. Sample weighting và average uniqueness
...
## 2.8. Bias-variance, baseline và lựa chọn mô hình
...
## 2.11. Rủi ro backtest overfitting và calibration
...
Bốn lớp này tạo nền tảng cho phương pháp ở Chương 3 và cách diễn giải kết quả ở Chương 4.
