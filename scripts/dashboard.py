...
def get_latest_session() -> Path | None:
    ...
    sessions = [
        d for d in results_dir.iterdir() if d.is_dir() and d.name.startswith("XAUUSD")
    ]
    if sessions:
...
def print_metrics_summary(metrics: dict | None) -> None:
    ...
    sharpe_color = (
        Colors.GREEN if sharpe > 1.5 else Colors.YELLOW if sharpe > 0 else Colors.RED
    )

    max_dd = metrics.get("max_drawdown_pct", 0)
    dd_color = (
        Colors.GREEN if max_dd < 20 else Colors.YELLOW if max_dd < 30 else Colors.RED
    )
    ...
    print(
        f"  {Colors.CYAN}Total Return:{Colors.RESET}     {return_color}{format_number(total_return)}%{Colors.RESET}"
    )
    print(
        f"  {Colors.CYAN}Sharpe Ratio:{Colors.RESET}     {sharpe_color}{format_number(sharpe)}{Colors.RESET}"
    )
    print(
        f"  {Colors.CYAN}Max Drawdown:{Colors.RESET}     {dd_color}{format_number(max_dd)}%{Colors.RESET}"
    )
    print(
        f"  {Colors.CYAN}Win Rate:{Colors.RESET}         {wr_color}{format_number(win_rate * 100, 1)}%{Colors.RESET}"
    )
    print(
        f"  {Colors.CYAN}Total Trades:{Colors.RESET}     {format_number(metrics.get('total_trades'), 0)}"
    )
    print(
        f"  {Colors.CYAN}Profit Factor:{Colors.RESET}    {format_number(metrics.get('profit_factor'))}"
    )
...
def print_session_files(session_path: Path) -> None:
    ...
    # Updated for simplified structure (LightGBM only, no LSTM)
    files_to_check = [
        ...
        ("Config Snapshot", "config/config_snapshot.toml"),
        ("Session Info", "config/session_info.json"),
    ]
...
def print_quick_commands(session_path: Path | None) -> None:
    ...
    commands = [
        ...
        ("Clean cache", "pixi run clean-cache"),
        ("Clean all", "pixi run clean-all"),
    ]
    ...
    if session_path:
        commands.extend(
            [
                ...
                (
                    "View backtest JSON",
                    f"cat {session_path}/backtest/backtest_results.json | jq '.metrics'",
                ),
            ]
        )
...
def print_health_check() -> None:
    ...
    # Check for 8 flat modules (simplified structure)
    required_modules = [
        "config.py",
        "features.py",
        "labels.py",
    ...
    src_dir = Path("src/thesis")
    module_status = []
    all_modules_exist = True

    for module in required_modules:
        exists = (src_dir / module).exists()
        module_status.append((module, exists))
        if not exists:
            all_modules_exist = False

    # Print module status
    print(f"\n  {Colors.CYAN}{Colors.BOLD}Core Modules (8 flat files):{Colors.RESET}")
    for module, exists in module_status:
        if exists:
    ...
    print(f"\n  {Colors.CYAN}{Colors.BOLD}Project Structure:{Colors.RESET}")
    all_good = all_modules_exist
...
def main() -> int:
    ...
    if latest_session:
        ...
        print(
            f"  {Colors.DIM}Run 'pixi run workflow' to create your first session{Colors.RESET}"
        )
    ...
    print(f"\n{Colors.BLUE}{Colors.BOLD}{'=' * 70}{Colors.RESET}")
    print(
        f"{Colors.DIM}Dashboard complete. Run 'python scripts/dashboard.py' anytime for updates.{Colors.RESET}\n"
    )
