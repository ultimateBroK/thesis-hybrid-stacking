...
def _add_pivot_points(
    ...
    # Merge back
    df = df.merge(
        daily[["date", "pivot_prev", "r1_prev", "s1_prev", "high_prev", "low_prev"]],
        on="date",
        how="left",
    )
...
def _add_microstructure_features(
    ...
    df["marubozu"] = (
        (df["upper_wick"] < 0.1 * df["range"]) & (df["lower_wick"] < 0.1 * df["range"])
    ).astype(int)
    ...
    df["body_wick_ratio"] = df["body"].abs() / (
        df["upper_wick"] + df["lower_wick"] + 1e-10
    )
