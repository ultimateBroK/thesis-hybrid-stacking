...
def _run_labels_stage(config: Config) -> None:
    ...
    if config.workflow.force_rerun or not output_path.exists():
        ...
        logger.info(f"  Output: {output_path}")
        logger.info(f"  Params: TP={config.labels.atr_multiplier_tp}×ATR, SL={config.labels.atr_multiplier_sl}×ATR, horizon={config.labels.horizon_bars} bars")
...
def _run_split_stage(config: Config) -> None:
    ...
    if config.workflow.force_rerun or not all([train_path.exists(), val_path.exists(), test_path.exists()]):
        logger.info("Splitting data into train/val/test...")
        logger.info(f"  Train: {config.splitting.train_start} → {config.splitting.train_end} (70%)")
        logger.info(f"  Val: {config.splitting.val_start} → {config.splitting.val_end} (15%) - Stress test (Russia-Ukraine + Fed hikes)")
        logger.info(f"  Test: {config.splitting.test_start} → {config.splitting.test_end} (15%) - New Regime (SVB + Gold ATH)")
        logger.info(f"  Purge: {config.splitting.purge_bars} bars | Embargo: {config.splitting.embargo_bars} bars")
...
def _run_lightgbm_stage(config: Config) -> None:
    ...
    if config.workflow.force_rerun or not model_path.exists():
        ...
        if config.models["tree"].use_optuna:
            logger.info(f"  Hyperparameter tuning: {config.models['tree'].optuna_trials} trials")
...
def _run_backtest_stage(config: Config) -> None:
    ...
    if config.workflow.force_rerun or not results_path.exists():
