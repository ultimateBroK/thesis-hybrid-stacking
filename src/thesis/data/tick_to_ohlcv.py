...
def validate_ohlcv_quality(df: pl.DataFrame) -> dict:
    ...
    if gaps:
        ...
        for gap in gaps[:3]:  # Show first 3
            logger.warning(f"    Gap: {gap['from']} → {gap['to']} ({gap['hours_missing']:.1f}h)")
    ...
    df_with_dow = df.with_columns(
        pl.col("timestamp").dt.weekday().alias("day_of_week")
    )
    weekend_bars = (df_with_dow["day_of_week"] > 5).sum()
    weekend_pct = weekend_bars / len(df) * 100
    if weekend_bars > 0:
        warnings.append(f"{weekend_bars} weekend bars ({weekend_pct:.1f}%) - expected minimal for forex")
