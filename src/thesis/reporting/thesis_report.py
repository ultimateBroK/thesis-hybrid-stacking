...
def _generate_model_disagreement_analysis(config: Config) -> None:
    ...
    if not lgbm_path.exists() or not lstm_path.exists():
        logger.warning(
            "Base model predictions not found, skipping disagreement analysis"
        )
        return
    ...
    axes[0].set_title(
        "Model Agreement (LightGBM vs LSTM)", fontsize=12, fontweight="bold"
    )
    ...
    bars = axes[1].bar(
        class_names, disagg_rates, color=["#e74c3c", "#f39c12", "#27ae60"]
    )
    axes[1].set_ylabel("Disagreement Rate (%)", fontsize=11)
    axes[1].set_title(
        "Model Disagreement by True Class", fontsize=12, fontweight="bold"
    )
...
def _generate_confidence_histogram(config: Config) -> None:
    ...
    axes[0, 0].set_title(
        "Overall Prediction Confidence Distribution", fontsize=12, fontweight="bold"
    )
    axes[0, 0].legend()
    ...
    axes[0, 1].set_title(
        "Confidence by Predicted Class", fontsize=12, fontweight="bold"
    )
    axes[0, 1].legend()
    ...
    axes[1, 0].axhline(
        0.6, color="red", linestyle="--", label="Trading Threshold (0.6)"
    )
    axes[1, 0].axhline(
        confidence.mean(),
        color="green",
    ...
    axes[1, 0].set_title(
        "Confidence Over Time (Sample)", fontsize=12, fontweight="bold"
    )
    axes[1, 0].legend()
    ...
    axes[1, 1].set_title(
        "Confidence Statistics Summary", fontsize=12, fontweight="bold"
    )
