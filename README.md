...
3. Chia tập theo thời gian (không xáo trộn): Train (2018–2023) / Val (2024–06/2025) / OOS (07/2025–03/2026)
4. Ngăn rò rỉ dữ liệu bằng Purging + Embargo
5. Xây dựng 20+ đặc trưng kỹ thuật + định lượng bằng Feature Importance / SHAP
6. Huấn luyện mô hình LSTM nền (chuỗi 60 nến OHLCV)
...
Raw Tick Data (Dukascopy)
  ...
  Feature Engineering (EMA, RSI, MACD, ATR, Pivot, Spread, Session)
        │
        ▼
  Gán nhãn Triple Barrier (TP=2×ATR×0.5, SL=1×ATR×0.5, h=10)
        │
        ▼
  Chia tập theo thời gian + Purging + Embargo (10–20 nến)
        │
        ├──────────────────┬──────────────────┐
        ▼                  ▼                  ▼
...
| **features** | Danh sách indicator + params | EMA(20,50,200), RSI(14), MACD(12,26,9), ATR(14), Pivot, Spread, Session |
| **labels** | `tp_atr_mult`, `sl_atr_mult`, `horizon`, `atr_coeff` | Triple Barrier: TP=2×ATR×0.5, SL=1×ATR×0.5, h=10 |
...
| **models.lstm** | Sequence length, hidden size, layers, dropout, epochs | `seq_len=60`, early stopping |
...
- [ ] Purging: loại mẫu ở vùng giao thoa khi gán nhãn (h=10)
- [ ] Embargo: chèn khoảng trống 10–20 nến giữa các tập
- [ ] Feature lookahead: đặc trưng chỉ dùng dữ liệu quá khứ (EMA, RSI, MACD...)
- [ ] LSTM window: kiểm tra cửa sổ 60 nến ở ranh giới tập
...
|------|-----------|-------|---------------------|
| Xu hướng | EMA(20, 50, 200) | Hướng giá | Giá đang tăng hay giảm? |
...
- **Take Profit:** `Close[t] + 2 × ATR × 0.5`
- **Stop Loss:** `Close[t] - 1 × ATR × 0.5`
- **Horizon:** h = 10 nến
- **Tỷ lệ R:R:** 2:1
